...
public class Sql implements Serializable, Cloneable{
	...
	public Sql page(int pageNumber, int pageSize) {
		...
		if(pageNumber==1&&pageSize==1&&dbType.equals(DBType.SQLSERVER)) {
	...
	public boolean isPostgresql() {
		return dbType.equals(DBType.POSTGRESQL);
	}
	/**
	 * 判断是oracle
	 * @return
	 ...
	 * @return
	 */
	public Sql orderBy(String key, String orderType) {
		this.orderBy = key;
		this.desc = orderType.equalsIgnoreCase("desc");
		return this;
	}
	/**
	 * 排序查询
	...
	public Sql leftJoin(String joinTableName,String asName,String onSql) {
		if(isOracle()) {
			this.joinSql = KEY_LEFT_JOIN + joinTableName+KEY_WHITESPACE+asName+KEY_ON+onSql;
		}else {
