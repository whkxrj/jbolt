...
public class GlobalConfigService extends JBoltBaseService<GlobalConfig> {
	...
	public Ret update(Object userId, GlobalConfig globalConfig) {
		...
		db.setConfigValue(globalConfig.getConfigValue()==null?"":globalConfig.getConfigValue());
	...
	public void checkAndInit(Object userId) {
		...
		checkAndInitConfig(userId, GlobalConfigKey.JBOLT_LOGIN_NEST);
		...
		checkAndInitConfig(userId, GlobalConfigKey.ASSETS_VERSION);
		checkAndInitConfig(userId, GlobalConfigKey.SYSTEM_ADMIN_H50);
		checkAndInitConfig(userId, GlobalConfigKey.SYSTEM_ADMIN_NAV_MENU_DEFAULT_ICON);
		checkAndInitConfig(userId, GlobalConfigKey.SYSTEM_ADMIN_GLOBAL_SEARCH_SHOW);
	}
	/**
	 * 检查并初始化全局配置表数据
	 * @param userId
	...
	public Ret changeAssetsVersion() {
		...
		return changeAssetsVersion(userId);
	}
	/**
	 * 更换 assets version
	 */
	public Ret changeAssetsVersion(Integer userId) {
