 ...
 */
public class WechatConfigService extends JBoltBaseService<WechatConfig> {
	...
	public Ret configOneMpinfo(WechatMpinfo mp) {
		...
		boolean isWxa=mp.getType().intValue()==WechatMpinfoType.XCX.getValue();
		...
		if(mp.getType()!=WechatMpinfoType.XCX.getValue()) {
		...
		if(isWxa){
	        ...
	        if(StrKit.notBlank(format)){
	        	switch (format) {
	        	case FORMAT_JSON:
	        		wxaConfig.useJsonMsgParser();
	        		break;
				case FORMAT_XML:
					wxaConfig.useXmlMsgParser();
					break;
				}
	        }else {
	        	wxaConfig.useXmlMsgParser();
