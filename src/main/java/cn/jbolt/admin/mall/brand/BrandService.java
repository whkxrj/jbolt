 ...
 */
public class BrandService extends JBoltBaseService<Brand> {
	...
	public Ret toggleEnable(Integer userId, Integer id) {
		...
		if(ret.isOk()){
			...
			return successWithData(brand.getEnable());
		}
		return ret;
	}
	/**
	 ...
	 * @return
	 */
	public Page<Brand> paginateAdminList(int pageNumber, int pageSize,String sortColumn,String sortType, String keywords) {
		if(notOk(keywords)) {
			return paginate(sortColumn, sortType, pageNumber, pageSize);
		}
		return paginateBySqlTemplate("mall.brand.paginateAdminList", Kv.by("keywords", keywords), pageNumber, pageSize,sortColumn,sortType);
	}
