 ...
 */
public class GoodsBackCategoryService extends JBoltBaseService<GoodsBackCategory> {
	...
	public List<JsTreeBean> getEnableTree(Integer checkedId) {
		return getEnableTree(checkedId, null);
	}
	/**
	 * 得到后台分类启动后的数据树
	 * @param checkedId 默认选中节点
	 * @param openType 打开类型 al||first_level
	 * @return
	 */
	public List<JsTreeBean> getEnableTree(Integer checkedId,String openType) {
		List<GoodsBackCategory> goodsCategories=getEnableList();
		return convertJsTreeDatas(goodsCategories,checkedId,openType);
	}
	/**
	 * 得到后台分类数据树 包含所有数据
	 * @param checkedId 默认选中节点
	 ...
	 * @return
	 */
	public List<JsTreeBean> getMgrTree(Integer checkedId, String openType) {
	...
	/**
	 * 转换数据成为jstreebean
	 * @param goodsBackCategories
	 * @param selectedId
	 * @param openType
	 * @return
	 */
	private List<JsTreeBean> convertJsTreeDatas(List<GoodsBackCategory> goodsBackCategories,Integer selectedId, String openType) {
		...
		if(goodsBackCategories.size()>0){
			...
			boolean firstLevelOpened=false;
			if(StrKit.notBlank(openType)) {
				if(openType.equalsIgnoreCase("all")) {
					allOpened=true;
				}else if(openType.equalsIgnoreCase("first_level")) {
					firstLevelOpened=true;
				}
			}
			boolean opened=false;
			for(GoodsBackCategory goodsCategory:goodsBackCategories){
				opened=allOpened||false;
				boolean selected=false;
				String type="default";
				if(opened==false&&notOk(goodsCategory.getPid())&&firstLevelOpened) {
					opened=true;
				}
