 ...
 */
public class GoodsService extends JBoltBaseService<Goods> {
	 ...
	 * @return
	 */
	public Page<Record> paginateAdminDataWithUserName(int pageNumber, int pageSize, String keywords,Boolean onSale,Boolean isHot,Boolean isRecommend,Boolean isDelete,Integer bcategoryId,Integer fcategoryId) {
		...
		Page<Record> page=dbTemplate("mall.goods.paginateAdminList2", paras).paginate(pageNumber, pageSize);
...
/*	*//**
	...
	public Ret toggleOnSale(Integer userId, Integer id) {
		...
		if(ret.isOk()){
			...
			return successWithData(goods.getOnSale());
		}
		return ret;
	}
	 ...
	 */
	@Override
	public String afterToggleBoolean(Kv kv,Goods goods, String column) {
		Date now=new Date();
	...
	public Ret toggleIsHot(Integer userId, Integer id) {
		Ret ret=toggleBoolean(Kv.by("userId", userId),id, "is_hot");
		if(ret.isOk()){
			Goods goods=ret.getAs("data");
			//添加日志
			addUpdateSystemLog(id, userId, SystemLog.TARGETTYPE_MALL_GOODS, goods.getName(), "[热销状态]:"+(goods.getIsHot()?"热销商品":"普通商品"));
			return successWithData(goods.getIsHot());
		}
		return ret;
	}
	/**
	...
	public Ret toggleIsRecommend(Integer userId, Integer id) {
		Ret ret=toggleBoolean(Kv.by("userId", userId),id, "is_recommend");
		if(ret.isOk()){
			Goods goods=ret.getAs("data");
			//添加日志
			addUpdateSystemLog(id, userId, SystemLog.TARGETTYPE_MALL_GOODS, goods.getName(), "[推荐状态]:"+(goods.getIsHot()?"已推荐":"未推荐"));
			return successWithData(goods.getIsRecommend());
		}
		return ret;
	}
	@Override
