...
public class JBoltParaValidator{
	...
	public static boolean isExcel(File file) {
		...
		if(isEndWithXls==false) {return false;}
		String type=FileTypeUtil.getType(file);
		if("xls".equalsIgnoreCase(type)==false) {return false;}
		return true;
	}
	/**
	 * 判断不是excel文件
	...
	public static boolean notExcel(File file) {
		if(file==null||file.exists()==false) {return true;}
		boolean notEndWithXls=(FileUtil.pathEndsWith(file, "xls")==false&&FileUtil.pathEndsWith(file, "xlsx")==false);
		if(notEndWithXls) {return true;}
		String type=FileTypeUtil.getType(file);
		if("xls".equalsIgnoreCase(type)==false) {return true;}
		return false;
	}
